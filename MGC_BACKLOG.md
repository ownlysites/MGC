...
| # | Item | Commit | Note |
|---|---|---|---|
| 32 | The visual pass: a desktop layout, and the tells removed | `565a2e4` | Both working screens had rendered in a 560px centred column at every viewport since launch — 44% of a 1280px monitor working, 56% idle. Now a sticky section rail plus a 768px content column above 1080px, collapsing to the old single column below it; the rail is built from the rendered DOM, so a new section with an `h2` joins it for free. Removed: side-stripe borders on six components, gradient text on the site's own headline (its middle stop ran ~2:1 on cream), emoji severity badges, and two consecutive grids of identical bordered cards — the four steps became a numbered sequence, the eight coverage items a ruled list. The landing hero led with a 480px logo alone on a full screen and put the proposition on the third scroll; headline and media are now one grid, no copy touched. Landing 5,250px → 4,064px. New `design.js` harness enforces all of it across six widths (58 checks). **Near-miss worth knowing:** the first CSS splice anchored on a comment that appears twice in the file and selected 1,200 lines; nothing was written because a later assertion failed first. Splices against index.html must count occurrences and bound the span. |
| 29 | Privacy opt-out letter | `8e48cdf` | The creditor opt-out form from the Dispute Plan folder, rebuilt. **Kept:** GLBA § 502 / 15 U.S.C. § 6802(b)(1) (nonaffiliated third parties); FCRA § 603(d)(2)(A)(iii) (affiliate sharing of anything beyond transactions and experiences — and the letter states the consequence, that the election removes the "consumer report" exclusion); FCRA § 624 / § 1681s-3(a)(1) with its five-year floor at (a)(3)(A). All four provisions read verbatim from the U.S. Code before writing. **Dropped, do not restore:** the transaction-and-experience affiliate opt-out (no such right — § 1681a(d)(2)(A)(i) and (ii) exclude it outright) and "silence for thirty days means you complied" (no statute creates it). **Added:** the § 604(e) prescreen opt-out below the signature — optoutprescreen.com / 1-888-567-8688, five years online or by phone versus permanent by signed Election form. Prefilled with real creditors and, where the report’s Creditor Contacts section gives a confident match, real addresses; other open accounts listed as separate sends rather than merged, since one letter naming two companies tells each about the other. Government and self-reported tradelines excluded — the first draft addressed one to the U.S. Department of Education. **Two envelope bugs fixed:** mailEnvelope closed every letter asking the recipient to confirm in writing and listing a photo ID as an enclosure; both are wrong for a letter that says no response is required and exists to move less personal information. Now suppressible per letter, suppressed only here. Scheduled day-zero in its own "Privacy instructions" group via a new template-level `send_class` — it is an instruction, not a dispute, so it does not wait fourteen days behind the bureau round. |
